import { type IllustrationFirework } from "./type/common"; // Uncomment if needed in the future

// // 2次元配列をローカルストレージに保存する関数
// export function saveArrayToLocalStorage(key: string, array: number[][]): void {
//   try {
...
// // ローカルストレージから2次元配列を読み込む関数
// export function loadArrayFromLocalStorage(key: string): number[][] | null {
//   try {
//     const jsonString = localStorage.getItem(key);
//     if (jsonString === null) {
//       console.log(`No data found in localStorage for key: ${key}`);
...
//     // 簡単な型チェック (より厳密なチェックも可能)
//     if (Array.isArray(parsedArray) && parsedArray.every(row => Array.isArray(row) && row.every(item => typeof item === 'number'))) {
//       console.log(`Array loaded from localStorage with key: ${key}`);
//       return parsedArray as number[][];
//     } else {
//       console.warn(`Data found for key "${key}" is not a valid 2D number array. Returning null.`);
//       localStorage.removeItem(key); // 不正なデータなら削除するのも手
//       return null;
//     }
//   } catch (error) {
