...
export default function FireworksDashboard() {
  ...
  const fetchFirework = async (id: number) => {
    try {
      ...
      setError(`Failed to fetch firework: ${err instanceof Error ? err.message : String(err)}`);
  ...
  const updateFirework = async (id: number, isShareable: boolean) => {
    setIsUpdating(true);
    try {
      const response = await fetch(`${API_URL}/fireworks/${id}`, {
        method: 'PUT',
      ...
      setError(`Failed to update firework: ${err instanceof Error ? err.message : String(err)}`);
  ...
  // Delete a firework
  const deleteFirework = async (id: number) => {
  ...
  const renderFireworkPreview = (pixelData: boolean[], size: number = 100) => {
                  ...
                  <table className="w-full">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{selectedFirework.pixelData.length}</td>
                      </tr>
                      <tr>
                        <td className="py-2 font-semibold">Active Pixels:</td>
                        <td>
                          {selectedFirework.pixelData.filter(Boolean).length}
                          ({Math.round(selectedFirework.pixelData.filter(Boolean).length / selectedFirework.pixelData.length * 100)}%)
                        </td>
                      </tr>
                    </tbody>
                  </table>
